...
module PL =
    ...
    let standardize formula = (formula, Formula.Extract formula |> Seq.distinct |> Seq.indexed) ||> Seq.fold (fun x (i, v) -> Formula.Replace(x, Variable(v), Variable($"%i{i}")))
